test_that("prioritize_cca_components returns Shape and Factor data frames", {
  ...
  rownames(shape_mat) <- rownames(nmf_mat) <- paste0("cell_", seq_len(n))
  ...
  if (nrow(res$Factor) > 0) expect_true(all(res$Factor$feature_name %in% colnames(nmf_mat)))
})
...
test_that("cell_subsample restricts the cell pool", {
  ...
  nmf_mat   <- scale(Z %*% matrix(rnorm(3 * 5), 3, 5) + matrix(rnorm(n * 5, sd = 0.3), n, 5))
  colnames(shape_mat) <- paste0("ShapePC", seq_len(6))
  colnames(nmf_mat)   <- paste0("Factor",  seq_len(5))
  rownames(shape_mat) <- rownames(nmf_mat) <- paste0("cell_", seq_len(n))
