...
#'   reduction keys to be unique across the object). Default \code{"NMF_"};
#'   the group name is appended to keep keys unique across groups, e.g.
#'   \code{"NMF_TypeA_1"}, \code{"NMF_TypeA_2"}, ...
#'
...
store_nmf_results <- function(obj,
  ...
  for (grp in names(nmf_results)) {
    ...
    reduction_key  <- paste0(reduction_key_prefix, grp, "_")
